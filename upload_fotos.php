...
function guardar_archivos($archivo, $key, $extensiones_permitidas) {
    ...
    $archivo_destino = $carpeta_destino . '/' .uniqid('foto_') .".$extension";
    ...
    if (move_uploaded_file($archivo_temporal, $archivo_destino)) {
        ...
        echo '<button class="btn-login" style="background-color: rgb(184, 230, 59); color: #fff; border: none; padding: 10px 20px; border-radius: 5px; font-size: 16px;"><a href="fotos.php" style="color: #fff; text-decoration: none;">Continuar</a></button>';
...
if (isset($_POST['submit'])) {
    ...
    foreach ($rutas_guardadas as $ruta) {
        $query = "INSERT INTO fotos (idPaciente, url) VALUES (?,?)";
